    ...
    <div class="container">
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb">
                ...
                @if($product->category)
                    <li class="breadcrumb-item"><a href="{{ route('products.index') }}?category={{ $product->category->cat_id }}">{{ $product->category->localized_name }}</a></li>
                @endif
        ...
        <div class="row g-5">
            <div class="col-lg-5">
                <div class="bg-light rounded-3 overflow-hidden position-relative" style="aspect-ratio: 1;">
                    @if($product->pro_image)
                        <img src="{{ asset('storage/' . $product->pro_image) }}" alt="{{ $product->localized_name }}" class="product-detail-image">
                    @else
            ...
            <div class="col-lg-7">
                <h1 class="fw-bold mb-2">{{ $product->localized_name }}</h1>
                @if($product->category)
                    <p class="text-muted mb-3"><i class="fas fa-folder me-1"></i>{{ $product->category->localized_name }}</p>
                @endif
                <div class="mb-4">
                    <span class="fs-3 fw-bold text-warning">HK${{ number_format($product->pro_price, 0) }}</span>
                </div>
                <p class="mb-4" style="line-height: 1.8; font-size: 1.05rem;">{{ $product->localized_desc }}</p>

                @if($product->pro_stock > 0)
                    <div class="mb-2 text-success">
                        <i class="fas fa-check-circle me-1"></i>{{ __('messages.in_stock') }}（{{ __('messages.stock') }} {{ $product->pro_stock }}）
                    </div>
                @else
                    <div class="mb-2 text-danger">
                        <i class="fas fa-times-circle me-1"></i>{{ __('messages.out_of_stock') }}
                    </div>
                @endif
